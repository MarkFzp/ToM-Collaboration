...
class PolicyNetwork:
    def __init__(self, config, state_encoding, agent_tensor, history_input):
        self.config_ = config
        self.state_encoding_ = state_encoding # Batch_Size X Sequence_len X dim
        ...
        self.components_concat_ = tf.concat([self.state_encoding_, self.agent_tensor_, self.history_encoding_], axis = 2)
        ...
        self.action_prob_unscale_ = tf.squeeze(tf.layers.conv2d(self.feat_tensors_[-1], self.config_.num_actions, kernel_size = 1,
