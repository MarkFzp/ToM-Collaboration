...
class Calendar(Game):
    def __init__(self, player1, player2,
        ...
        self.action_tensor_ = np.concatenate([self.all_msgs_tensor_, np.zeros([self.num_msgs_, self.num_slots_])], axis = 1)
        ...
        self.meetable_slots_tensor_ = []
        for i in range(self.num_calendars_):
            inner_meetable = []
            for j in range(self.num_calendars_):
                inner_meetable.append(self.tensor_[i] + self.tensor_[j] == 0)
            self.meetable_slots_tensor_.append(np.array(inner_meetable))
        self.meetable_slots_tensor_ = np.array(self.meetable_slots_tensor_)
    ...
    def reset(self, player1_cid = None, player2_cid = None, train = True):
        ...
        self.meetable_slots_ = self.meetable_slots_tensor_[self.player1_cid_, self.player2_cid_]
        
        return

    def start(self):
        return {self.player1_name_: edict({'observation': None, 'private': self.tensor_[self.player1_cid_], 'terminate': False}),
                self.player2_name_: edict({'observation': None, 'private': self.tensor_[self.player2_cid_], 'terminate': False})}
...
def test_overlap():
    ...
    for idx in range(1, 4):
        dataset_idx = idx
        calendar = Calendar('A', 'B', 0, -1, 1, num_slots, os.path.join(base_path, 'calendar_{}_{}'.format(num_slots, dataset_idx)))
