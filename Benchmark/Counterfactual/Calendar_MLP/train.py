...
def sample_trajs():
    ...
    for _ in range(config.batch_size if not config.test else config.test_batch_size):
        ...
        traj = {'p1_calendar': p1_calendar, 'p2_calendar': p2_calendar, 'fst_act_player': fst_act_player, \
            'reward': [], 'pi': [], 'prev_action_p_idx': [], 'action': [], 'traj_len': None, \
            'in_x_1': [], 'in_x_2': []}
        ...
        while not terminate:
            if t >= config.terminate_len:
                ...
                if t % 2 == fst_act_player:
                    ...
                    traj['prev_action_p_idx'].append(prev_action_p_idx)
...
def fit_qnet(trajs, max_traj_len):
    ...
    for traj in trajs:
        ...
        p1_calendars.extend([traj['p1_calendar']] * traj_len)
        p2_calendars.extend([traj['p2_calendar']] * traj_len)
        prev_action_p_idxs.extend(traj['prev_action_p_idx'])
        # q_chosen_idxs.append(np.stack([
        #     np.arange(q_gather_row, q_gather_row + traj_len, dtype = np.int32),
        #     np.array(traj['action'])
        # ], axis = 1))
        q_chosen_col_idxs.extend(traj['action'])
        q_gather_row += traj_len
    
    p1_calendar_np = np.stack(p1_calendars, axis = 0)
    p2_calendar_np = np.stack(p2_calendars, axis = 0)
    prev_action_p_idx_np = np.stack(prev_action_p_idxs, axis = 0)
    q_chosen_idx_np = np.stack([range(q_gather_row), q_chosen_col_idxs], axis = 1)

    target_q_value = qnet_target.get_q(p1_calendar_np, p2_calendar_np, prev_action_p_idx_np, q_chosen_idx_np)
    ...
    for traj in trajs:
        ...
        for epoch, curr_q_target_row in zip(reversed(range(traj_len)), reversed(range(q_target_row, q_target_row + traj_len))):
            if epoch == traj_len - 1:
                ...
                td_lambda_value = \
                    ...
                    config.gamma * ( \
                        ...
                        + \
                        (1 - config.td_lambda) * target_q_value[curr_q_target_row + 1] \
                    )
                td_lambda_values[epoch] = td_lambda_value
        
        q_target_row += traj_len
        
        td_lambda_value_l.extend(td_lambda_values)

    td_lambda_value_np = np.array(td_lambda_value_l)
    
    loss = qnet_primary.fit_target_q(p1_calendar_np, p2_calendar_np, prev_action_p_idx_np, q_chosen_idx_np, td_lambda_value_np)
    primary_q_value = qnet_primary.get_all_q(p1_calendar_np, p2_calendar_np, prev_action_p_idx_np)
...
def policy_grad(trajs, max_traj_len, primary_q_value):
    ...
    for i, traj in enumerate(trajs):
        ...
        in_xs.append(in_x_2)
        action = traj['action']
        traj_q = primary_q_value[q_gather_row: q_gather_row + traj_len]
...
def main():
    ...
    for episode in range(config.ctd_itr if config.ctd_itr is not None else 0, config.itr):
