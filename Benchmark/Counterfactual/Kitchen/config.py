...
config.load_path = 'test_aaai_td0.5_entropy0.05_{}_1.ckpt'.format(split) if not snd_round \
    else 'test_aaai_td0.5_entropy0.05_{}_2.ckpt'.format(split)
...
config.ckpt_path_2 = 'test_{}_1/test_aaai_td0.5_entropy0.05_{}_2.ckpt'.format(split, split) if not snd_round \
    else 'test_{}_2/test_aaai_td0.5_entropy0.05_{}_1.ckpt'.format(split, split)
config.dataset_path = os.path.join(dataset_path, 'dataset{}_with_10_ingred_4_dish_5_maxingred_1000000_size_0.7_ratio.npz'.format(split))
...
config.fc_layer_dims = [40, 40, config.num_action]  #[100, 100, config.num_action]
