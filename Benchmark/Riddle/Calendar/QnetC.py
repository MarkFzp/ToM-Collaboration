...
class QNet:
    def __init__(self, config, sess, target=False):
        ...
        self.num_fingerprint = self.config.num_fingerprint
        ...
        with tf.variable_scope(self.scope_name):
            ...
            self.fingerprint = tf.placeholder(tf.float32, [None, None, self.num_fingerprint])
            self.obv_with_fingerprint = tf.concat([self.obv, self.fingerprint], axis=2)
            ...
            if config.ad_hoc_structure:
                ...
                self.obv_ = tf.concat([self.obv_with_fingerprint, self.hidden_states], -1)
    ...
    def save_ckpt(self, sess, global_step):
    ...
    def get_q(self, obv, fingerprint, hidden_states, index):
    ...
    def fit_target_q(self, obv, fingerprint, hidden_states, player_index, action_chosen, target_q, q_values_mask):
        train_opt, loss, q_predicted = self.sess.run([self.train_opt, self.loss, self.q_values], feed_dict={
            self.obv: obv,
            self.fingerprint: fingerprint,
            self.hidden_states: hidden_states,
            self.index: player_index,
