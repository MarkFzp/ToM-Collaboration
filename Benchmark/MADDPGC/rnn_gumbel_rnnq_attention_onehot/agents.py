...
from Benchmark.MADDPGC.rnn_gumbel_rnnq_attention_onehot.policy import *

def p_func(inp, in_state, calendar_tensor, act_dim, scope, num_units, trainable=True, gpu=True):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):

        if gpu:
            cell = tf.keras.layers.CuDNNGRU(calendar_tensor.get_shape().as_list()[-2], return_sequences = True, return_state = True, trainable=trainable)
        else:
            cell = tf.keras.layers.GRU(calendar_tensor.get_shape().as_list()[-2], return_sequences = True, return_state = True, trainable=trainable)
        ...
        L = tf.concat([attended_calendar, inp], axis=-1)
...
def build_net(name, num_agents, index, calendar_tensor_,action_tensor_, num_slots, num_ensemble, act_dim, p_func, q_func, num_units,
    ...
    last_act_ph_n_policy_ = [tf.one_hot(act, act_dim) for act in last_act_ph_n_policy]

    # obs_ph_policy = calendar_tensor
    obs_ph = tf.concat([private_ph] + last_act_ph_n_policy_, axis=-1)

    in_state_ph = tf.placeholder_with_default(tf.zeros([tf.shape(private_ph)[0], calendar_tensor.get_shape().as_list()[-2]]),
                                              [None, calendar_tensor.get_shape().as_list()[-2]])
    q_in_state_ph = tf.placeholder_with_default(tf.zeros([tf.shape(private_ph)[0], calendar_tensor.get_shape().as_list()[-2]]),
                                                [None, calendar_tensor.get_shape().as_list()[-2]])
    ...
    policy
    '''
    inp = {'calendar_tensor':calendar_tensor, 'action_tensor':action_tensor, 'p_input':obs_ph, 'in_state':in_state_ph,
    ...
    ## for critic
    q_input = tf.concat([obs_ph] + [tf.one_hot(act, act_dim) for act in act_ph_n_q], axis=-1)
