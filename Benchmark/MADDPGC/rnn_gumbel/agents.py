...
def p_func(inp, in_state, calendar_tensor, act_dim, scope, num_units, reg, trainable=True, gpu=True):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        L = tf.layers.dense(out_seq, num_units, activation=tf.nn.leaky_relu, kernel_regularizer=tf.contrib.layers.l2_regularizer(reg),
                            bias_regularizer=tf.contrib.layers.l2_regularizer(reg), trainable=trainable)
        L = tf.layers.dense(L, num_units, activation=tf.nn.leaky_relu,  kernel_regularizer=tf.contrib.layers.l2_regularizer(reg),
                            bias_regularizer=tf.contrib.layers.l2_regularizer(reg),trainable=trainable)
        # L = tf.layers.dense(L, num_units, activation=tf.nn.leaky_relu, trainable=trainable)
        # L = tf.layers.dense(L, num_units, activation=tf.nn.leaky_relu, trainable=trainable)
        L = tf.layers.dense(L, act_dim,  kernel_regularizer=tf.contrib.layers.l2_regularizer(reg),
                            bias_regularizer=tf.contrib.layers.l2_regularizer(reg),trainable=trainable)

    vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=scope)
    return [L, out_state], vars

def q_func(inp, q_dim, scope, num_units, reg, trainable=True, gpu=True):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        L = tf.layers.dense(inp, num_units, activation=tf.nn.leaky_relu,  kernel_regularizer=tf.contrib.layers.l2_regularizer(reg),
                            trainable=trainable)
        L = tf.layers.dense(L, num_units, activation=tf.nn.leaky_relu,  kernel_regularizer=tf.contrib.layers.l2_regularizer(reg),
                            trainable=trainable)
        L = tf.layers.dense(L, num_units, activation=tf.nn.leaky_relu,  kernel_regularizer=tf.contrib.layers.l2_regularizer(reg),
        ...
        # L = tf.layers.dense(L, num_units, activation=tf.nn.leaky_relu, trainable=trainable)
        # L = tf.layers.dense(L, num_units, activation=tf.nn.leaky_relu, trainable=trainable)
        L = tf.layers.dense(L, q_dim,  kernel_regularizer=tf.contrib.layers.l2_regularizer(reg),
                            trainable=trainable)
...
def build_net(name, num_agents, index, calendar_tensor_,action_tensor_, num_slots, num_ensemble, act_dim, p_func, q_func, num_units,
    ...
    placeholder
    '''
    private_ph_n = [tf.placeholder(tf.float32, [None, None, num_slots]) for _ in range(num_agents)]
    ...
    calendar_tensor = tf.tile(tf.constant(calendar_tensor_[1:], tf.float32)[tf.newaxis,  tf.newaxis],
                              (tf.shape(private_ph_n[index])[0], tf.shape(private_ph_n[index])[1], 1, 1))  ## ignore the first calendar slot?
    ...
    obs_ph_q = tf.concat([tf.cast(sum(private_ph_n) > 0, tf.float32)]+last_act_ph_n_policy_, axis=-1)
    obs_ph_pol = tf.concat([private_ph_n[index]]+last_act_ph_n_policy_, axis=-1)


    in_state_ph = tf.placeholder_with_default(tf.zeros([tf.shape(private_ph_n[index])[0], num_units]),
                                              [None, num_units])
    ...
    policy
    '''
    inp = {'calendar_tensor':calendar_tensor, 'action_tensor':action_tensor, 'p_input':obs_ph_pol, 'obs_ph_q':obs_ph_q, 'in_state':in_state_ph, 'mask':mask_ph, 'temp':temp_ph, 'act_ph_n_q':act_ph_n_q, 'target_q_ph':target_q_ph}
    ...
    ## for critic
    q_input = tf.concat([obs_ph_q] + [tf.gather(action_tensor,act) for act in act_ph_n_q], axis=-1)
    c_q, train_q_vars = q_func(q_input, 1, scope=name+"_train_q", num_units=num_units, reg=1e-2, trainable=True)

    c_loss = tf.reduce_mean(tf.square(tf.boolean_mask(c_q, mask_ph) - tf.boolean_mask(target_q_ph, mask_ph)))

    ## target
    target_q, target_q_vars = q_func(q_input, 1, scope=name+"_target_q", num_units=num_units, reg=1e-2, trainable=False)
    ...
    return {'ph': [private_ph_n, last_act_ph_n_policy, act_ph_n_q, target_q_ph, temp_ph, in_state_ph, mask_ph],
            'actor': policy,
...
class MADDPGAgentTrainer:
    def __init__(self, name, num_agents, agent_index, config, actor_model, critic_model, calendar_tensor, action_tensor):
        ...
        self.private_ph_n, self.last_act_ph_n_policy, self.act_ph_n_q, self.target_q_ph, self.temp_ph, self.in_state_ph, self.mask_ph = dict['ph']
    ...
    def action(self, sess, private, last_action_n:list, temp, mask=np.array([[True]]), in_state=None,  train=True):
        ...
        feed = dict(zip([self.private_ph_n[self.agent_index], self.temp_ph, self.mask_ph]+self.last_act_ph_n_policy,[private,temp, mask]+last_action_n))
    ...
    def update_pi(self, sess, private_n: list, last_action_n:list, action_n:list, mask, temp):
        # train p network
        #
        # q = self.get_q(sess, menu, workplace_embed, action, goal, train=False)

        feed_dict = dict(zip(self.private_ph_n+[self.temp_ph, self.mask_ph]+self.last_act_ph_n_policy +self.act_ph_n_q,
                             private_n+[temp, mask]+last_action_n+action_n))
    ...
    def update_q(self, sess, agents, private_n:list, last_action_n:list, action_n:list, rewards, terminal, mask_all_agents:list, temp):
        # train q network

        ## td lambda

        bs, traj_len = private_n[0].shape[:2]

        ## get td lambda
        all_target_act = [agent.action(sess, private_n[l], last_action_n, temp, ~terminal, train=False)[0] for l, agent in enumerate(agents)]
        for act,m in zip(all_target_act, mask_all_agents):
            act[~m] = -1.
        qvalues = [self.get_q(sess,
                              [private[:,i:i+1,:] for private in private_n], [last_act[:,i:i+1] for last_act in last_action_n],
                              [act[:, i:i + 1] for act in all_target_act],
                              train=False) for i in range(traj_len)]
        r = rewards[:,-1:]
        td_target = [r]
        ...
        if self.config.break_correlation:
            newbs = private_n[0].shape[0]*private_n[0].shape[1]
            ind = np.random.choice(newbs, size=newbs//2, replace=False)
            private_n_ = [private.reshape((newbs,1,private.shape[2]))[ind] for private in private_n]
            ...
            private_n_ = private_n
            td_target_ = td_target
            terminal_ = terminal
            last_action_n_ = last_action_n
            action_n_ = action_n

        feed_dict = dict(zip(self.private_ph_n+[self.target_q_ph, self.mask_ph]+self.last_act_ph_n_policy+self.act_ph_n_q,
                             private_n_+[td_target_, ~terminal_]+last_action_n_+action_n_))
    ...
    def update(self, sess, agents, temp, t):
        ...
        q_loss, target_q  = \
            self.update_q(sess, agents, private_n, last_act_n, act_n, rewards, terminal, mask_all_agents, temp)

        # train p network
        p_loss = self.update_pi(sess, private_n, last_act_n, act_n, my_mask, temp)
    ...
    def get_q(self, sess, private_n:list, last_action_n:list, act_n:list, train=True):
        ...
        q = sess.run(self.train_q if train else self.target_q,
                          feed_dict=dict(zip(self.private_ph_n+self.last_act_ph_n_policy+self.act_ph_n_q,
                                             private_n+last_action_n+act_n)))
        return q

    def get_all_action_q(self, sess, private_n:list, last_action_n:list, train=True):
        q = []
        bs, t = private_n[0].shape[0:2]
        for i in range(self.config.num_actions):
            act = np.full((bs,t), i)
            act_n = [np.full(act.shape, -1) if k != self.agent_index else act for k in range(self.n)]
            q_ = self.get_q(sess, private_n,last_action_n, act_n, train)
            q.append(q_)
