...
from Benchmark.MADDPGC.rnn_gumbel.buffer import ReplayBuffer
...
class Policy:
    ...
    def _build_net(self, inp, act_dim, num_units):
        ...
        obs_ph_q = inp['obs_ph_q']


        ensemble = []
        for i in range(self.K):

            [logits, out_state], train_p_vars = self.p_func(p_input, in_state_ph, calendar_tensor, act_dim, self.name+'_train_p_%d'%i, num_units, 1e-2, True, self.use_gpu)
            ...
            act_sample_cont = tf.reduce_sum(act_sample_cont[:, :, :, tf.newaxis] * action_tensor[tf.newaxis, tf.newaxis], axis=-2)

            ## loss

            q_input = tf.concat([obs_ph_q]+[tf.gather(action_tensor, act) if i!=self.agent_index else
                                           act_sample_cont
                                           for i, act in enumerate(act_ph_n_q)], axis=-1)
            q, _ = self.q_func(q_input, 1, scope=self.train_q_scope, num_units=num_units, reg=1e-2, trainable=True)
            ...
            a_train_opt = optimizer.minimize(a_loss, var_list=train_p_vars)
            ...
            [target_logits, target_out_state], target_p_vars = self.p_func(p_input, in_state_ph, calendar_tensor, act_dim,
                                                            self.name + '_target_p_%d' % i, num_units, 1e-2, False,
                                                            self.use_gpu)
