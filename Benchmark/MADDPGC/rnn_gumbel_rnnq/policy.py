...
from Benchmark.MADDPGC.rnn_gumbel_rnnq.buffer import ReplayBuffer
...
class Policy:
    ...
    def _build_net(self, inp, act_dim, num_units):
        ...
        for i in range(self.K):
            ...
            act_sample_cont = tf.tile(private_ph, [1,1,2]) - tf.reduce_sum(
            ...
            q_input = tf.concat([p_input]+[tf.tile(private_ph, [1,1,2]) - tf.gather(action_tensor, act) if i!=self.agent_index else
                                           act_sample_cont
            ...
            lr = tf.train.exponential_decay(self.a_lr, self.global_step, self.lr_decay_step, self.lr_decay_rate,
                                            staircase=True)
            ...
            ensemble.append({'train':{'logits':logits, 'train_p_vars':train_p_vars, 'act_sample_cont':act_sample_cont,
                             ...
                             'target': {'logits': target_logits, 'train_p_vars': target_p_vars,
                                        ...
                                        'prob': target_prob},
                             'opt':{'loss':a_loss, 'train_opt':a_train_opt, 'optimizer':optimizer},
