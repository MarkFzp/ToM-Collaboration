...
from Benchmark.MADDPGC.rnn_pg.policy import *
...
def q_func(inp, q_dim, scope, num_units, trainable=True, gpu=True):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
...
def build_net(name, num_agents, index, calendar_tensor_,action_tensor_, num_slots, num_ensemble, act_dim, p_func, q_func, num_units,
    ...
    # calendar_tensor = tf.tile(tf.transpose(tf.constant(calendar_tensor_[1:], tf.float32),
    #                                        [1, 0])[tf.newaxis,  tf.newaxis],
    ...
    # calendar_encoding = tf.reduce_sum(calendar_tensor * calendar_encoding, axis=-1)
    ...
    act_ph_n_q_ = [tf.tile(tf.cast(sum(private_ph_n)>0, tf.float32), [1, 1, 2]) - tf.gather(action_tensor, act) for i, act in enumerate(act_ph_n_q)]
    last_act_ph_n_policy = [tf.placeholder(tf.int32, [None, None]) for _ in range(num_agents)]
    obs_ph = tf.concat([tf.tile(tf.cast(sum(private_ph_n)>0, tf.float32), [1, 1, 2]) - tf.gather(action_tensor, act)
                             for act in last_act_ph_n_policy], axis=-1)
    p_inp = tf.concat([tf.tile(private_ph_n[index], [1, 1, 2]) - tf.gather(action_tensor, act)
                             for act in last_act_ph_n_policy], axis=-1)
    ...
    policy
    '''
    inp = {'action_tensor':action_tensor, 'p_input':p_inp, 'obs_ph':obs_ph, 'in_state':in_state_ph, 'rstd_ph':rstd_ph, 'rm_ph':rm_ph,
            'mask':mask_ph, 'act_ph_n_q':act_ph_n_q, 'target_q_ph':target_q_ph, 'private_ph_n':private_ph_n}
    ...
    ## for critic
    q_input = tf.concat([obs_ph] + act_ph_n_q_, axis=-1)
    c_q, train_q_vars = q_func(q_input, 1, scope=name+"_train_q", num_units=num_units, trainable=True, gpu=use_gpu)

    c_loss = tf.reduce_mean(tf.square(tf.boolean_mask(c_q, mask_ph) - tf.boolean_mask(target_q_ph, mask_ph)))

    ## target
    target_q, target_q_vars = q_func(q_input, 1, scope=name+"_target_q", num_units=num_units, trainable=False, gpu=use_gpu)
    ...
    return {'ph': [private_ph_n, last_act_ph_n_policy, act_ph_n_q, target_q_ph, rm_ph, rstd_ph, in_state_ph, mask_ph],
            'actor': policy,
...
class MADDPGAgentTrainer:
    def __init__(self, name, num_agents, agent_index, config, actor_model, critic_model, calendar_tensor, action_tensor):
        ...
        self.private_ph_n, self.last_act_ph_n_policy, self.act_ph_n_q, self.target_q_ph, self.rm_ph, self.rstd_ph, self.in_state_ph, self.mask_ph = dict['ph']
    ...
    def action(self, sess, private, last_action_n:list, mask=np.array([[True]]), in_state=None,  train=True):
        assert self.random_policy_index is not None

        feed = dict(zip([self.private_ph_n[self.agent_index], self.mask_ph]+self.last_act_ph_n_policy,[private, mask]+last_action_n))
    ...
    def update_pi(self, sess, private_ph_n, last_action_n:list, action_n:list, rm, rstd, mask):
        # train p network
        #
        # q = self.get_q(sess, menu, workplace_embed, action, goal, train=False)

        feed_dict = dict(zip(self.private_ph_n+[self.mask_ph, self.rm_ph, self.rstd_ph]+self.last_act_ph_n_policy +self.act_ph_n_q,
                             private_ph_n+[mask, rm, rstd]+last_action_n+action_n))
    ...
    def update_q(self, sess, agents, private_ph_n, last_action_n:list, action_n:list, rewards, terminal, mask_all_agents:list):
        # train q network

        ## td lambda

        bs, traj_len = private_ph_n[0].shape[:2]

        ## get td lambda
        all_target_act = [agent.action(sess, private_ph_n[l], last_action_n, ~terminal, train=False)[0] for l, agent in
                          enumerate(agents)]
        for act, m in zip(all_target_act, mask_all_agents):
            act[~m] = -1.
        qvalues = [self.get_q(sess, [private[:, i:i + 1] for private in private_ph_n],
                                [lact[:, i:i+1] for lact in last_action_n],
        ...
        for i in reversed(range(traj_len - 1)):
            q_ = qvalues[i + 1]
            r = rewards[:, i:i + 1] + self.config.gamma * (self.config.lamb * r + (1 - self.config.lamb) * q_) * (
                        1. - terminal[:, i + 1:i + 2])
        ...
        tdmean = np.mean(discounted_r[~terminal])
        tdstd = np.std(discounted_r[~terminal])

        # Break correlation
        if self.config.break_correlation:
            newbs = private_ph_n[0].shape[0]*private_ph_n[0].shape[1]
            ind = np.random.choice(newbs, size=newbs//2, replace=False)
            private_n_ = [private.reshape((newbs,1,private.shape[2]))[ind] for private in private_ph_n] #menu
    ...
    def update(self, sess, agents, t):
        ...
        q_loss, target_q, rm, rstd   = \
            self.update_q(sess, agents, private_n, last_act_n, act_n, rewards, terminal, mask_all_agents)

        # train p network
        p_loss = self.update_pi(sess, private_n, last_act_n, act_n, rm, rstd , my_mask)
    ...
    def get_all_action_q(self, sess, private_ph_n, last_action_n:list, train=True):
        q = []
        bs, t = private_ph_n[0].shape[0:2]
        for i in range(self.config.num_actions):
            act = np.full((bs,t), i)
            act_n = [np.full(act.shape, -1) if k != self.agent_index else act for k in range(self.n)]
            q_ = self.get_q(sess, private_ph_n,last_action_n, act_n, train)
            q.append(q_)
