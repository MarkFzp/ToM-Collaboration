...
from Benchmark.MADDPGC.rnn_pg.buffer import ReplayBuffer
...
class Policy:
    ...
    def _build_net(self, inp, act_dim, num_units):
        ...
        for i in range(self.K):
            ...
            q_input = tf.concat([obs_ph]+[tf.tile(tf.cast(sum(private_ph_n)>0, tf.float32), [1, 1, 2]) - tf.gather(action_tensor, act)
                                           for i, act in enumerate(act_ph_n_q)], axis=-1)
            q, _ = self.q_func(q_input, 1, scope=self.train_q_scope, num_units=num_units, trainable=True, gpu=self.use_gpu)
            ...
            a_loss = m_pg + self.beta * (reg - entropy)
