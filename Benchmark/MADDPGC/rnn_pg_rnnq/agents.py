...
from Benchmark.MADDPGC.rnn_pg_rnnq.policy import *
...
def build_net(name, num_agents, index, calendar_tensor_,action_tensor_, num_slots, num_ensemble, act_dim, p_func, q_func, num_units,
    ...
    act_ph_n_q_ = [tf.gather(action_tensor, act) for i, act in enumerate(act_ph_n_q)]
    ...
    obs_ph = tf.concat([private_ph]+last_act_ph_n_policy_, axis=-1)
    ...
    policy
    '''
    inp = {'action_tensor':action_tensor, 'p_input':obs_ph, 'in_state':in_state_ph, 'rstd_ph':rstd_ph, 'rm_ph':rm_ph,
           'q_in_state_ph':q_in_state_ph, 'mask':mask_ph, 'act_ph_n_q':act_ph_n_q, 'target_q_ph':target_q_ph, 'private_ph':private_ph}
    ...
    [c_q, _], train_q_vars = q_func(q_input, q_in_state_ph, 1, scope=name+"_train_q", num_units=num_units, trainable=True, gpu=use_gpu)

    c_loss = tf.reduce_mean(tf.square(tf.boolean_mask(c_q, mask_ph) - tf.boolean_mask(target_q_ph, mask_ph)))

    ## target
    [target_q, _], target_q_vars = q_func(q_input, q_in_state_ph, 1, scope=name+"_target_q", num_units=num_units, trainable=False, gpu=use_gpu)
    ...
    return {'ph': [private_ph, last_act_ph_n_policy, act_ph_n_q, target_q_ph, rm_ph, rstd_ph, in_state_ph, mask_ph],
            'actor': policy,
...
class MADDPGAgentTrainer:
    def __init__(self, name, num_agents, agent_index, config, actor_model, critic_model, calendar_tensor, action_tensor):
        ...
        self.private_ph, self.last_act_ph_n_policy, self.act_ph_n_q, self.target_q_ph, self.rm_ph, self.rstd_ph, self.in_state_ph, self.mask_ph = dict['ph']
    ...
    def action(self, sess, private, last_action_n:list, mask=np.array([[True]]), in_state=None,  train=True):
        ...
        feed = dict(zip([self.private_ph, self.mask_ph]+self.last_act_ph_n_policy,[private, mask]+last_action_n))
    ...
    def update_pi(self, sess, private, last_action_n:list, action_n:list, rm, rstd, mask):
        # train p network
        #
        # q = self.get_q(sess, menu, workplace_embed, action, goal, train=False)

        feed_dict = dict(zip([self.private_ph, self.mask_ph, self.rm_ph, self.rstd_ph]+self.last_act_ph_n_policy +self.act_ph_n_q,
                             [private, mask, rm, rstd]+last_action_n+action_n))
        _, p_loss, adv = sess.run([self.policy[self.random_policy_index]['opt']['train_opt'],
    ...
    def update_q(self, sess, agents, private, last_action_n:list, action_n:list, rewards, terminal, mask_all_agents:list):
        # train q network
        ...
        ## get td lambda
        all_target_act = [agent.action(sess, private, last_action_n,  ~terminal, train=False)[0] for agent in agents]
    ...
    def update(self, sess, agents, t):
        ...
        q_loss, target_q, rm, rstd   = \
            self.update_q(sess, agents, private, last_act_n, act_n, rewards, terminal, mask_all_agents)

        # if t < 1e4:
        #     return [q_loss, None, np.mean(target_q[my_mask]), np.mean(rewards[my_mask]), np.std(target_q[my_mask]), np.sum(~terminal, axis=-1).mean(), None]


        # train p network
        p_loss, adv = self.update_pi(sess, private, last_act_n, act_n, rm, rstd , my_mask)

        if t % self.config.update_step == 0:
            self.q_update(sess)
            self.p_update(sess)

        return [q_loss, p_loss, np.mean(target_q[my_mask]), np.mean(rewards[my_mask]), np.std(target_q[my_mask]),  np.sum(~terminal, axis=-1).mean(), adv]
