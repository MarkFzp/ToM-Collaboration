...
from Benchmark.MADDPGC.rnn_pg_rnnq.test import *
        ...
        for ep in range(init_ep, MAX_EP):
                ...
                for j in range(private_traj_log[0].shape[1]):
                    ...
                    logging.info(
                        ...
                        '   Top 3 Policy: {}, {}\n'.format(
                            ...
                            private, reward_traj[chosen][j][0], q, logit, ent_traj[chosen][j],
            ...
            for l, agent in enumerate(agents):
                ...
                if result is not None and ep % LOG_STEP == 0:
                    q_loss, p_loss, target_q_mean, reward_mean, target_q_std_mean, traj_le, adv = result

                    logging.info(
                        '       Name: {}\n      Q_Loss: {} P_Loss: {}\n'
                        '       Mean Target Q: {} Mean Target Q std: {}\n'
                        '       Mean Reward: {} Mean Traj Len: {} Adv: {}\n'.format(
                            agent.name, q_loss, p_loss, target_q_mean, target_q_std_mean,
                            reward_mean, traj_le, adv))
