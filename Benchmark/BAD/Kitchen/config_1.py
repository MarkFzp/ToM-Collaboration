...
split = 1
snd_round = False
config.dataset_path = '/home/luyao/Datasets/Kitchen/dataset{}_with_12_ingred_7_dish_5_maxingred_1000000_size_0.7_ratio.npz'.format(split)
config.continue_itr = None
#####

config.load_ckpt = 'test_{}_1/test_{}_1'.format(split, split) if not snd_round \
    else 'test_{}_2/test_{}_2'.format(split, split)
config.load_ckpt_2 = 'test_{}_2/test_{}_2'.format(split, split) if not snd_round \
    else 'test_{}_1/test_{}_1'.format(split, split)
config.ckpt_path = 'test_{}_2/test_{}_2'.format(split, split) \
    if snd_round else 'test_{}_1/test_{}_1'.format(split, split)
...
config.policy_fc_dim = [100, 100, config.num_action] #[40, 40, config.num_action]
config.policy_context_dim = [(8, 14)]  #[(6, 12)] [(8, 14)]
config.workplace_fc_dim = 8
...
config.value_fc_dim = [100, 100, 1] #[40, 40, 1]
