...
from Benchmark.MADDPG.rnn_pg.config import *
from Benchmark.MADDPG.rnn_pg.agents import *
        ...
        for ep in range(MAX_EP):
            ...
            for t in range(MAX_TIME):
                ...
                for i, name in enumerate(['human', 'robot']):
                    a, in_state[i], ent, logit = agents[i].action(sess, menu[:,None,:,:], wpeb[:,None,:], goal, temp,
                                                        in_state=in_state[i], train=True)
                    _, in_state[(i+1)%2], _, _ = agents[(i+1)%2].action(sess, menu[:, None, :, :], wpeb[:, None, :], goal, temp,
                    ...
                    if ep % LOG_STEP == 0:
                        ...
                        logging.info(
                            ...
                            '   Top 3 Q: {}, Top 3 Policy: {}\n'.format(
                                name, a, menu, goal, wpeb, next_obs[name].reward, q, logit, ent, temp,
            ...
            if acc > max_acc:
                ...
                saver.save(sess, os.path.join(ckpt_dir,'best','best_model.ckpt'), ep)
