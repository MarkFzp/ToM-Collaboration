...
from Benchmark.MADDPG.rnn_gumbel.buffer import ReplayBuffer
...
class Policy:
    ...
    def _build_net(self, inp, act_dim, num_units):
        ...
        for i in range(self.K):
            ...
            q_input = tf.concat([p_input]+[tf.one_hot(act, act_dim) if i!=self.agent_index else act_sample_cont for i, act in enumerate(act_ph_n)], axis=-1)
            ...
            # lr = tf.train.exponential_decay(self.a_lr, self.global_step, self.lr_decay_step, self.lr_decay_rate,  staircase=True)
