...
def build_net(name, num_agents, index, num_dishes, num_ingredients, num_ensemble, act_dim, p_func, q_func, num_units,
    ...
    policy = Policy(name, num_agents, index, num_ensemble, p_func, q_func,inp, act_dim, num_units, a_lr, init_global_step,
                    lr_decay_step, lr_decay_rate, beta,
    ...
    return {'ph': [menu_ph, wpeb_ph, goal_ph, act_ph_n, target_q_ph, temp_ph, in_state_ph, mask_ph],
            'actor': policy,
...
class MADDPGAgentTrainer:
    def __init__(self, name, num_agents, agent_index, config, actor_model, critic_model):
        ...
        self.menu_ph, self.wpeb_ph, self.goal_ph, self.act_ph_n, self.target_q_ph, self.temp_ph, self.in_state_ph, self.mask_ph = dict['ph']
