...
class QNet:
    def __init__(self, config, sess, qnet_name_appendix, target = False):
        ...
        with tf.variable_scope(self.variable_scope_name):
            ...
            self.is_test = tf.placeholder(tf.bool, shape = [])
            self.is_const_mess_test = tf.placeholder(tf.bool, shape = [])
            ...
            for t in range(config.max_attribute):
                ...
                message = tf.cond(
                    self.is_const_mess_test, 
                    lambda: self.default_message, 
                    lambda: self.messages[-1]
                )
                ...
                action_argmax = lambda: tf.cast(tf.argmax(q, axis = 1), tf.int32)
                action_egreedy = lambda: tf.stop_gradient(tf.where(
                    self.unif_rand[:, t] < self.epsilon_1d,
                    self.action_uniform_dis.sample(config.batch_size),
                    tf.cast(tf.argmax(q, axis = 1), tf.int32)
                ))
                action_fed = lambda: self.action_fed[:, t]
                action_ = lambda: tf.cond(self.is_test, action_argmax, action_egreedy)
                action = tf.cond(self.is_action_fed, action_fed, action_)
    ...
    def train(self, menu, p1_target, action_fed, q_spvs, q_idx):
        loss, epsilon, _ = self.sess.run(
            [self.loss, self.epsilon, self.train_op],
            feed_dict = {
                self.menu: menu,
                self.p1_target: p1_target,
                self.is_action_fed: True,
                self.is_test: False,
                self.is_const_mess_test: False, 
                self.action_fed: action_fed, 
                self.q_spvs: q_spvs,
                self.q_idx: q_idx
            }
        )
    ...
    def get_target_q(self, menu, p1_target, action_fed):
        ...
        if self.config.test or self.config.change_pair_test:
            action = self.sess.run(
                ...
                feed_dict = {
                    self.menu: menu,
                    self.p1_target: p1_target,
                    self.is_action_fed: False, 
                    self.is_test: True, 
                    self.is_const_mess_test: False
                }
            )
        elif self.config.constant_message_test:
            action = self.sess.run(
                self.action_tensor,
                feed_dict = {
                    self.menu: menu,
