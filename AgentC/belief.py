...
class BeliefNetwork:
    def __init__(self, config, num_actions, background, state, history_input, initial_belief):
        self.config_ = config
        self.background_ = background # (2 ** num_slots) X num_slots
        ...
        assert(self.belief_dim_ == self.background_.shape[0])
        self.num_slots_ = int(np.log2(self.belief_dim_))
        ...
        self.components_.append(tf.expand_dims(tf.expand_dims(self.background_, 0), 0) *\
                                tf.ones_like(tf.expand_dims(self.history_encoding_, -1))[:, :, 0: 1, 0: 1])
        ...
        self.likelihood_pre_ = tf.layers.conv2d(self.feat_tensors_[-1], self.num_actions_, kernel_size = 1,
        ...
        self.new_belief_raw_ = tf.transpose(self.new_belief_pre_ / tf.reduce_sum(self.new_belief_pre_,
                                                                                 axis = 2, keepdims = True),
                                            [0, 1, 3, 2])
        # Batch_Size X Sequence_len X num_actions X num_slots
        self.new_belief_ = tf.reduce_sum(tf.expand_dims(self.new_belief_raw_, -1) *\
                           tf.expand_dims(tf.expand_dims(tf.expand_dims(self.background_, 0), 0), 0), axis = 3)

def main():
    from easydict import EasyDict as edict
    config = edict({'history_dim': 20, 'fc_layer_info': [30, 20]})
    B_net = BeliefNetwork(config, tf.random.uniform(shape = [256, 8]), tf.random.uniform(shape = [4, 5, 20]),
                          tf.random.uniform(shape = [4, 5, 10]), tf.random.uniform(shape = [4, 1, 256]))
    sess = tf.Session()
